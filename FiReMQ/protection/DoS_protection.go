...
/*
	Объяснение RateLimitMiddleware(rate, burst):
	rate — скорость пополнения: как часто появляется новый "токен" (разрешение на запрос)
	burst — размер "корзины": сколько токенов может накопиться, пока никто не отправляет запросов. Это максимум запросов, которые можно отправить подряд мгновенно, после чего действует основной ритм
	Пример: rate.Every(5*time.Second), 2 — обычно 1 запрос каждые 5 секунд, но если админ не нажимал кнопку 10+ секунд, накопится 2 токена, и можно нажать 2 раза подряд.
*/
