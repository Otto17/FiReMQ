...
// Обработчики для кнопки "Включить/Выключить"
document.getElementById("toggleReserveAccount").addEventListener("click", toggleReserveAccountState);
document.getElementById("toggleReserveAccount").addEventListener("contextmenu", handleReserveAccountContextMenu);
...
function toggleReserveAccountState(e) {
  ...
  // Если кнопка в режиме подтверждения, левый клик — отмена
  if (toggleButton.classList.contains("confirm-mode")) {
    if (e && e.button === 0) {
...
// Обработчик правого клика для резервного аккаунта
function handleReserveAccountContextMenu(e) {
  const toggleButton = document.getElementById("toggleReserveAccount");
  if (toggleButton.classList.contains("confirm-mode")) {
    e.preventDefault();
...
function resetReserveAccountButtonState() {
  ...
  // Устанавливает текст кнопки в зависимости от текущего состояния аккаунта
  if (reserveAccountState.textContent === "Включён") {
...
// Запрашивает актуальное состояние резервного аккаунта с сервера и обновляет UI
function refreshReserveAccountFromServer() {
  fetch("/get-accounts-mqtt")
    .then(response => response.json())
    .then(data => {
      const reserveAccount = data.find(acc => acc.account === 1);
      if (reserveAccount) {
        refreshReserveAccountUI(reserveAccount.allow);
      }

      // Обновляет текущий логин на случай смены приоритетов
      const account0 = data.find(acc => acc.account === 0);
      if (account0) {
        document.getElementById("currentMqttUsername").value = account0.username;
      }
    ...
    .catch(error => {
      console.error("Ошибка обновления состояния резервного аккаунта:", error);
    });
}

// Обновляет отображение состояния резервного аккаунта без перезагрузки окна
function refreshReserveAccountUI(allow) {
  const reserveAccountState = document.getElementById("reserveAccountState");
  const toggleReserveAccountButton = document.getElementById("toggleReserveAccount");
...
function closeMqttAuthModal() {
  ...
  // Принудительный сброс состояния кнопок при закрытии
  ...
  apiPostJson("/update-account-mqtt", {
    ...
    .then(async (response) => {
      ...
      if (response.ok) {
        ...
        // Обновляет состояние резервного аккаунта (сервер мог включить его автоматически)
...
function toggleSaveButtonState(e) {
  const saveButton = document.getElementById("saveMqttAuth");
  const usernameInput = document.getElementById("mqttUsername");
  const passwordInput = document.getElementById("mqttPassword");

  // Если кнопка УЖЕ в режиме подтверждения, значит левый клик — это отмена
...
// Обработчик правого клика для кнопки "Сохранить"
function handleMqttSaveContextMenu(e) {
  const saveButton = document.getElementById("saveMqttAuth");
  if (saveButton.classList.contains("confirm-mode")) {
    e.preventDefault();
    saveMqttAuth();
...
function resetSaveMqttButtonState() {
  const saveButton = document.getElementById("saveMqttAuth");
  const usernameInput = document.getElementById("mqttUsername");
  const passwordInput = document.getElementById("mqttPassword");
...
document.getElementById("saveMqttAuth").addEventListener("contextmenu", handleMqttSaveContextMenu);
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Привязка события к кнопке "Статус"
  const showStatusBtn = document.getElementById("showMqttStatusBtn");
  if (showStatusBtn) {
    showStatusBtn.addEventListener("click", showMqttAuthStatusModal);
  }
});
...
// Текущий фильтр статусов (all, pending, success, error)
let mqttStatusCurrentFilter = "all";

// Кэш данных клиентов для фильтрации без повторного запроса
let mqttStatusClientsCache = {};
...
function loadMqttAuthStatus() {
  fetch("/mqtt-auth-status")
    .then(response => response.json())
    .then(data => {
      if (!data.success) {
      ...
      updateMqttStatusUI(data);
    })
    .catch(error => {
      console.error("Ошибка запроса статусов:", error);
    });
}

// Обновление интерфейса статусов
function updateMqttStatusUI(data) {
  const clientsList = document.getElementById("mqttStatusClientsList");
  const resendBtn = document.getElementById("mqttResendErrorsBtn");
  ...
  document.getElementById("statError").textContent = data.stats?.error || 0;

  // Управляет кнопкой повторной отправки
  const hasErrors = (data.stats?.error || 0) > 0;
  resendBtn.disabled = !hasErrors;

  // Если все клиенты успешно обновились, обновляет состояние резервного аккаунта на заднем плане
  if (data.active && data.stats?.total > 0 && data.stats?.pending === 0 && data.stats?.error === 0) {
    refreshReserveAccountFromServer();
  }
  ...
  document.getElementById("mqttStatusUsername").textContent = `Новый логин: ${data.new_username || "—"}`;

  // Сохраняет данные в кэш для фильтрации
  const clients = data.clients || {};
  mqttStatusClientsCache = clients;

  // Рендерит список с учётом текущего фильтра
  renderFilteredClientsList();
}

// Рендеринг списка клиентов с учётом фильтра
function renderFilteredClientsList() {
  const clientsList = document.getElementById("mqttStatusClientsList");
  const clients = mqttStatusClientsCache;
  let clientIDs = Object.keys(clients);

  if (clientIDs.length === 0) {
    clientsList.innerHTML = '<div class="mqtt-status-empty">Нет клиентов</div>';
    return;
  }

  // Фильтрует по текущему фильтру
  if (mqttStatusCurrentFilter !== "all") {
    clientIDs = clientIDs.filter(id => clients[id].status === mqttStatusCurrentFilter);
  }

  if (clientIDs.length === 0) {
    const filterText = getFilterText(mqttStatusCurrentFilter);
    clientsList.innerHTML = `<div class="mqtt-status-empty">Нет клиентов со статусом "${filterText}"</div>`;
    return;
  }

  // Сортирует клиентов: сначала ошибки, потом ожидающие, потом успешные
  const statusOrder = {
    error: 0,
    pending: 1,
    success: 2
  ...
  clientIDs.sort((a, b) => {
    const statusA = statusOrder[clients[a].status] ?? 3;
    const statusB = statusOrder[clients[b].status] ?? 3;
    if (statusA !== statusB) return statusA - statusB;
    return (clients[a].client_name || a).localeCompare(clients[b].client_name || b);
  });

  // Проверяет, нужно ли полностью перестроить список
  const existingItems = clientsList.querySelectorAll(".mqtt-client-item");
  const existingIDs = Array.from(existingItems).map(el => el.dataset.clientId);
  const needsRebuild = existingIDs.length !== clientIDs.length ||
    !clientIDs.every((id, idx) => existingIDs[idx] === id);

  if (needsRebuild) {
    // Полная перестройка списка
    clientsList.innerHTML = clientIDs.map(clientID => createClientItemHTML(clientID, clients[clientID])).join("");
  } else {
    // Обновляет только статусы существующих элементов
    existingItems.forEach(item => {
      const clientID = item.dataset.clientId;
      const clientData = clients[clientID];
      if (!clientData) return;

      updateClientItemStatus(item, clientData);
    });
  }
}

// Получение текста фильтра для сообщения
...
function handleStatFilterClick(event) {
  const target = event.target.closest(".stat-item");
  if (!target) return;

  const filter = target.dataset.filter;
  if (!filter) return;

  // Если кликнули на уже активный фильтр — сбрасываем на "all"
  if (mqttStatusCurrentFilter === filter && filter !== "all") {
    mqttStatusCurrentFilter = "all";
  } else {
    mqttStatusCurrentFilter = filter;
  }
...
function updateFilterButtonsState() {
  const statItems = document.querySelectorAll(".mqtt-status-stats .stat-item");
  statItems.forEach(item => {
    const filter = item.dataset.filter;
    if (filter === mqttStatusCurrentFilter) {
...
// Создание HTML для элемента клиента
function createClientItemHTML(clientID, clientData) {
  const statusClass = `status-${clientData.status}`;
  const badgeClass = clientData.status;
  const statusText = getStatusText(clientData.status);

  let errorMsg = "";
  if (clientData.status === "error" && clientData.message) {
    errorMsg = `<div class="mqtt-client-error-msg">${escapeHtml(clientData.message)}</div>`;
  }

  return `
		<div class="mqtt-client-item ${statusClass}" data-client-id="${escapeHtml(clientID)}">
			<div class="mqtt-client-info">
				<span class="mqtt-client-name">${escapeHtml(clientData.client_name || clientID)}</span>
				<span class="mqtt-client-id">${escapeHtml(clientID)}</span>
			</div>
			<div class="mqtt-client-status">
				<span class="mqtt-status-badge ${badgeClass}">${statusText}</span>
				<span class="mqtt-client-time">${escapeHtml(clientData.updated_at || "")}</span>
				${errorMsg}
			</div>
		</div>
	`;
...
// Обновление статуса существующего элемента клиента (без перестройки DOM)
function updateClientItemStatus(item, clientData) {
  // Обновляет классы статуса
  item.className = `mqtt-client-item status-${clientData.status}`;

  // Обновляет бейдж
  const badge = item.querySelector(".mqtt-status-badge");
  if (badge) {
    badge.className = `mqtt-status-badge ${clientData.status}`;
    badge.textContent = getStatusText(clientData.status);
  }

  // Обновляет время
  const timeEl = item.querySelector(".mqtt-client-time");
  if (timeEl) {
    timeEl.textContent = clientData.updated_at || "";
  }

  // Обновляет сообщение об ошибке
  let errorEl = item.querySelector(".mqtt-client-error-msg");
  if (clientData.status === "error" && clientData.message) {
    if (!errorEl) {
      errorEl = document.createElement("div");
      errorEl.className = "mqtt-client-error-msg";
      item.querySelector(".mqtt-client-status").appendChild(errorEl);
    }
    errorEl.textContent = clientData.message;
  } else if (errorEl) {
    errorEl.remove();
  }
}
...
// Повторная отправка команд клиентам с ошибками
function resendMqttAuthToErrors() {
  const resendBtn = document.getElementById("mqttResendErrorsBtn");
  resendBtn.disabled = true;
  ...
  apiPostJson("/mqtt-auth-resend", {})
    .then(async response => {
      ...
      if (response.ok && data.success) {
        ...
        // Немедленно обновляет статусы
        loadMqttAuthStatus();
      } else {
        showPush(data.message || "Ошибка отправки", "#ff4081"); // Розовый
      }
    })
    .catch(error => {
    ...
    .finally(() => {
      ...
      // Кнопка будет включена/выключена при следующем обновлении статусов
    });
}

// Очистка сессии смены авторизации (с подтверждением)
function clearMqttAuthSession(e) {
  const clearBtn = document.getElementById("mqttClearSessionBtn");

  // Если уже в режиме подтверждения, левый клик — отмена
  if (clearBtn.classList.contains("confirm-mode")) {
    if (e && e.button === 0) {
      resetClearSessionButton();
    }
    return;
  ...
  // Активирует режим подтверждения
  clearBtn.textContent = "Подтверди правым кликом!";
  clearBtn.classList.add("confirm-mode");
}

// Обработчик правого клика для очистки сессии
function handleClearSessionContextMenu(e) {
  const clearBtn = document.getElementById("mqttClearSessionBtn");
  if (clearBtn.classList.contains("confirm-mode")) {
    e.preventDefault();
    performClearSession();
...
// Сброс состояния кнопки очистки
function resetClearSessionButton() {
  const clearBtn = document.getElementById("mqttClearSessionBtn");
  clearBtn.textContent = "Очистить сессию";
  clearBtn.classList.remove("confirm-mode");
}

// Привязка событий к элементам модального окна статуса MQTT
document.getElementById("closeMqttStatusModal")?.addEventListener("click", closeMqttAuthStatusModal);
document.getElementById("mqttResendErrorsBtn")?.addEventListener("click", resendMqttAuthToErrors);
document.getElementById("mqttClearSessionBtn")?.addEventListener("click", clearMqttAuthSession);
document.getElementById("mqttClearSessionBtn")?.addEventListener("contextmenu", handleClearSessionContextMenu);

// Привязка обработчика фильтрации к статистике
document.querySelector(".mqtt-status-stats")?.addEventListener("click", handleStatFilterClick);
...
// Возвращает true, если в данный момент нет возможности начать новую загрузку
function isBlocked() {
...
// Сигнатура состояния статуса клиента для cmd/PowerShell (чтобы не перерисовывать без надобности)
function getCmdStatusSig(cd) {
  return [
    normalizeStr(cd.Answer),
    normalizeStr(cd.Cmd_Execution),
    normalizeStr(cd.Description)
  ].join('|');
}

// Генерация HTML статуса для "По cmd / PowerShell"
function buildCmdStatusHTML(clientData) {
  const hasAnswer = !!(clientData && clientData.Answer && clientData.Answer.trim());
  if (!hasAnswer) {
    return '<span class="pending">—</span>';
  }

  const answer = (clientData.Answer || '').trim();
  const execution = (clientData.Cmd_Execution || '').trim();

  // При успехе выводит только галочку и время, без тултипа
  if (answer === 'success') {
    return `<span class="done">✓ - ${execution}</span>`;
  }
  // При ошибке выводит крестик, время и ТОЛЬКО описание ошибки (для краткости тултипа)
  else if (answer === 'error') {
    const descr = (clientData.Description || '').trim();
    const tip = `${descr || 'Ошибка исполнения'}`;
    return `<span class="error tt-btn tt-wide" data-tt="${escapeAttr(tip)}">X - ${execution}</span>`;
  }
  ...
  fetch('/get-terminal-report', {
    ...
    .then(data => {
      ...
      if (selectedCommand) {
        ...
        displayCmdClients(selectedCommand.ClientID_Command, selectedDate);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('closeTerminalOutputModal').addEventListener('click', closeTerminalOutputModal);
  document.getElementById('closeTerminalOutputModalBtn').addEventListener('click', closeTerminalOutputModal);
  document.getElementById('copyTerminalOutputBtn').addEventListener('click', copyTerminalOutput);
...
function displayCmdClients(clients, dateCreationParam) {
  ...
  const selectedDate = dateCreationParam || document.getElementById('cmdDateSelect').value;
    for (const [clientId, clientData] of Object.entries(clients)) {
      ...
      row.innerHTML = `
            ...
            <td>${statusHTML}</td>
            <td>
				<button class="action-button view tt-btn" data-client-id="${clientId}" data-tt="Показать полный вывод запроса">ⅈ</button>
...
// Подсветка синтаксиса консоли (для CMD и PowerShell)
function highlightTerminalSyntax(text) {
  if (!text) return "";

  // Нормализация служебных символов
  text = String(text)
    .replace(/\r/g, '')
    .replace(/\x08/g, '');
  ...
  // Проверяет только НАЧАЛО строки на ключевые слова ошибки/предупреждения
  const errorLineRegex = /^(error|ошибка|exception|fail|failed|access denied|отказано в доступе|сбой)[\s:]/i;
  const warningLineRegex = /^(warning|предупреждение|warn)[\s:]/i;

  return lines.map((line) => {
    // Проверяет, начинается ли строка с ключевого слова ошибки или предупреждения
    if (errorLineRegex.test(line.trim())) {
      return `<span class="term-error">${escapeHTML(line)}</span>`;
    }
    if (warningLineRegex.test(line.trim())) {
      return `<span class="term-warn">${escapeHTML(line)}</span>`;
    }

    // Определяет тип prompt и разделяем строку
    let promptPart = "";
    let rest = line;
    let hasPrompt = false;
    let promptClass = "";
    ...
    if (/^PS\s+[a-zA-Z]:\\/.test(line) && line.includes('>')) {
      const p = line.indexOf('>');
      promptPart = line.slice(0, p + 1);
      rest = line.slice(p + 1);
      hasPrompt = true;
      promptClass = "term-ps-prompt";
      shellKind = "ps";
    }
    // CMD prompt: C:\path>
    ...
    // Сборка строки
    if (hasPrompt) {
      let promptRendered = escapeHTML(promptPart);
      promptRendered = promptRendered.replace(
        /(PS\s+)?([a-zA-Z]:\\[^&]*?)(&gt;)/,
        (m, ps, path, gt) => {
          const psPart = ps ? ps : "";
          return `${psPart}<span class="term-path">${path}</span><span class="term-op">${gt}</span>`;
        }
      );
      return `<span class="${promptClass}">${promptRendered}</span>${renderedRest}`;
    }
...
// Токенизация строки и применение подсветки
function tokenizeAndHighlight(text, shellKind, isCommandLine) {
  if (!text) return "";
  ...
  while (i < text.length) {
    ...
    // Строки в кавычках (с учётом экранирования \")
    if (ch === '"') {
      let j = i + 1;
      while (j < text.length) {
        if (text[j] === '\\' && j + 1 < text.length) {
          j += 2; // Пропускает экранированный символ
          continue;
        }
        if (text[j] === '"') {
          j++;
    ...
    // Двоеточие как отдельный токен (для времени 14:06:30)
    if (ch === ':') {
      tokens.push({
        type: 'punct',
        text: ':'
    ...
    // Точка как отдельный токен (для дат и окончаний предложений)
    if (ch === '.') {
      tokens.push({
        type: 'punct',
        text: '.'
    ...
    // Многосимвольные операторы перенаправления: 2>&1, 1>&2
    const fourChar = text.slice(i, i + 4);
    if (/^[12]>&[12]/.test(fourChar)) {
      tokens.push({
        type: 'op',
        text: fourChar
      });
    ...
    // Числа: отрицательные (-42) только если минус отделён пробелом или пунктуацией слева
    if (ch === '-' && i + 1 < text.length && /[0-9]/.test(text[i + 1])) {
      const prevToken = tokens.length > 0 ? tokens[tokens.length - 1] : null;
      const isPrevSpace = !prevToken || prevToken.type === 'ws' || prevToken.type === 'op' || prevToken.type === 'punct';

      if (isPrevSpace) {
        let j = i + 1;
        while (j < text.length && /[0-9]/.test(text[j])) j++;
        if (j < text.length && text[j] === '.' && j + 1 < text.length && /[0-9]/.test(text[j + 1])) {
          j++;
          while (j < text.length && /[0-9]/.test(text[j])) j++;
        }
        tokens.push({
    ...
    if (/[0-9]/.test(ch)) {
      let j = i;
      while (j < text.length && /[0-9]/.test(text[j])) j++;
      // Десятичная точка только если за ней цифра
      if (j < text.length && text[j] === '.' && j + 1 < text.length && /[0-9]/.test(text[j + 1])) {
        j++;
        while (j < text.length && /[0-9]/.test(text[j])) j++;
      }
    ...
    while (j < text.length) {
      const c = text[j];
      // Останавливается на пробелах, операторах, кавычках, пунктуации
      if (c === ' ' || c === '\t' || c === '"' || c === "'" ||
        c === '|' || c === '>' || c === '<' || c === '&' ||
        c === ':' || c === '.' || c === ',') break;
      // Останавливается на ключах, если перед ними пробел
      if ((c === '/' || c === '-') && j > 0 && (text[j - 1] === ' ' || text[j - 1] === '\t')) break;
      // Останавливается, если встретили цифру (чтобы отделить числа в конце слов)
      if (/[0-9]/.test(c) && j > i) {
        // Проверяет, не является ли это частью слова типа "file10"
        // Если перед цифрой буква, продолжает
        if (!/[a-zA-Zа-яА-ЯёЁ]/.test(text[j - 1])) break;
      }
      j++;
    }
    ...
    if (j > i) {
      ...
      // Одиночный символ, который не попал в другие категории
      tokens.push({
        type: 'word',
        text: ch
      });
  ...
  // Определяет, какой токен является командой (подсветка команды только для строк с prompt "isCommandLine")
  ...
  return tokens.map(tok => {
    switch (tok.type) {
      ...
      case 'string':
        return `<span class="term-string">${escapeHTML(tok.text)}</span>`;
      case 'op':
        return `<span class="term-op">${escapeHTML(tok.text)}</span>`;
      case 'switch':
        return `<span class="term-switch">${escapeHTML(tok.text)}</span>`;
      case 'number':
        return `<span class="term-number">${escapeHTML(tok.text)}</span>`;
      case 'word':
        const t = tok.text;
        ...
        // Первое слово — команда (только в строках с prompt)
        if (!firstWordFound && isCommandLine) {
          firstWordFound = true;
        ...
        const trailingNumberMatch = t.match(/^(.+?)(\d+)$/);
        if (trailingNumberMatch) {
          const textPart = trailingNumberMatch[1];
          const numPart = trailingNumberMatch[2];
          return `${escapeHTML(textPart)}<span class="term-number">${escapeHTML(numPart)}</span>`;
        }
...
// Открывает модальное окно с результатом выполнения
function viewTerminalOutput(clientId, dateCreation) {
  const modal = document.getElementById('terminalOutputModal');
  const textArea = document.getElementById('terminalOutputText');

  // Сбрасывает содержимое перед загрузкой
  textArea.innerHTML = "Загрузка...";
  modal.style.display = "flex";

  fetch(`/get-terminal-client-info?date_of_creation=${encodeURIComponent(dateCreation)}&client_id=${encodeURIComponent(clientId)}`, {
      method: "GET",
      credentials: "same-origin"
    })
    .then(res => res.json())
    .then(data => {
      if (data.status === "success" && data.data) {
        ...
        const ans = data.data.Answer || "Unknown";

        let finalText = "";
        // Заголовок остаётся ТОЛЬКО если это ошибка (error), чтобы было понятно, что скрипт упал
        if (ans === "error") {
          finalText += `[ERROR] Ошибка выполнения:\n\n`;
        }

        finalText += desc.trim(); // Убирает пустые переносы в начале и конце

        // Применяет синтаксическую подсветку и вставляет как HTML
        textArea.innerHTML = highlightTerminalSyntax(finalText);
      } else {
        textArea.innerHTML = "Не удалось загрузить данные.";
      }
    })
    .catch(error => {
      console.error('Ошибка загрузки:', error);
      textArea.innerHTML = "Ошибка соединения с сервером: " + error.message;
    });
}

// Функция закрытия модального окна результата выполнения
function closeTerminalOutputModal() {
  const modal = document.getElementById('terminalOutputModal');
...
// Копирует текст из окна вывода терминала в буфер обмена
function copyTerminalOutput() {
  const pre = document.getElementById('terminalOutputText');
  if (!pre) return;

  // Получает текст без HTML-тегов
  const textToCopy = pre.innerText || pre.textContent;

  navigator.clipboard.writeText(textToCopy)
    .then(() => {
      showPush("Текст скопирован в буфер обмена", "#4CAF50"); // Зелёный
    })
    .catch(err => {
      console.error("Ошибка копирования:", err);
      showPush("Не удалось скопировать текст", "#ff4d4d"); // Красный
    });
}
