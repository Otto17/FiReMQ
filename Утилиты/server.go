...
type Server struct {
	...
	onPublishHandler func(clientID, topic string, clientIP string, payload []byte)	// Эта функция будет выполнена при публикации сообщений (КАСТОМ)
}
...
func (s *Server) publishToSubscribers(pk packets.Packet) {
	...
	// Если обработчик установлен и клиент-отправитель известен, вызываем обработчик только один раз
    if s.onPublishHandler != nil {
        if cl, ok := s.Clients.Get(pk.Origin); ok {
			ipOnly, _, err := net.SplitHostPort(cl.Net.Remote) 					// Разделяем строку на IP-адрес и порт
			if err != nil {														// Обработка ошибки, если адрес некорректный
				s.onPublishHandler(cl.ID, pk.TopicName, "Ошибка", pk.Payload)	// Передаём: ID клиента, топик, ошибку IP адреса и сообщение
				return 															// Завершаем выполнение, если произошла ошибка
			}	
            s.onPublishHandler(cl.ID, pk.TopicName, ipOnly, pk.Payload) // Передаём: ID клиента, топик, IP адрес и сообщение
        }
    }
}
...
// Добавил код в 148 строке и с 1025 по 1035 строки в этом файле.


// SetOnPublishHandler возвращает ID клиента (отправившего сообщение), название топика и сообщение из топика
func (s *Server) SetOnPublishHandler(handler func(clientID, topic string, clientIP string, payload []byte)) {
    s.onPublishHandler = handler
}

// GetID возвращает ID клиентов, подключенных к брокеру
